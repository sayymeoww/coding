/*
    Перед вами квадратная матрица ячеек. В каждой ячейке находится одно целое
   число. Одинаковых чисел в матрице нет. Вам надо разделить матрицу на
   непересекающиеся цепочки.

    Цепочкой называется такая последовательность различных ячеек, в которой две
   соседние ячейки имеют общую сторону и числа в этой последовательности «почти»
   возрастают. Это означает, что можно исключить не более 1 числа из
   последовательности, чтобы получилась возрастающая последовательность.

    Цепочки пересекаются, если имеют общую ячейку.
    Не должно быть двух цепочек, ячейки которых можно объединить и получить
   новую цепочку.

    Сначала идет размерность матрицы — целое число от 1 до 100. Затем идут
   элементы матрицы по строкам. Каждое число по модулю не превосходит 10000.

    Распечатайте одно из разбиений матрицы на цепочки. Сначала распечатайте
   количество цепочек. Затем печатайте каждую цепочку на отдельной строке.
   Цепочку требуется выводить в формате - число элементов в цепочки, затем сами
   элементы Числа в строке печатайте в десятичном виде через пробелы.
*/
...
while (moves < n * n) {
    ...
    if ((int)s.size() == 0) s.push_back(f[posx][posy]);

    bool flag = false;

    vector<coord> cells =
        neighbours(posx, posy, n, visited);  // получаем соседей
    ...
    if ((int)cells.size() != 0) {
        ...
        // если ничего не добавили - заносим змейку в ответ и ищем новые
        // координаты
        if (!flag) {
            answer.push_back(s);
            s.clear();
