   ...
   public:
            ...
            for (auto coord : coords)
                for (int i = coord.first - r; i <= coord.first + r; i++)
                    for (int j = coord.second - r; j <= coord.second + r; j++)
                        if (checkEdge(i, j, this->n, this->m) &&
                            distance(i, j, coord.first, coord.second) <= r) {
                            edges[i][j] = 1;
                            visited[i][j] = true;
                        }
    }
    ...
    void print() {
        for (int i = 0; i < this->n; i++) {
            for (int j = 0; j < this->m; j++) cout << edges[i][j] << " ";
            cout << '\n';
        }
    }
...
bool solve(board b, int x, int y) {
    if (y == b.getm() - 1 && b.edges[x][y] == 0) return true;
    b.visited[x][y] = true;
