...
class tree:
    ...
    def build(self, current: node):
        ...
        if self.height == 1:
            ...
            current.childLeft.parents.add((1, 0))
            current.childRight.parents.add((1, 1))
        ...
        if (
            ...
            # for el in current.parents:
            #    current.childLeft.parents.add(el)
            current.childLeft.parents = current.parents.copy()
            current.childLeft.parents.add(
                (current.childLeft.row, current.childLeft.column)
            )
        ...
        if (
            ...
            # for el in current.parents:
            #    current.childRight.parents.add(el)
            current.childRight.parents = current.parents.copy()
            current.childRight.parents.add(
    ...
    def lca(self, first: node, second: node) -> (int, int):
        f = self.find(first).parents
        s = self.find(second).parents

        if len(f) > len(s):
            return self.lca(second, first)
